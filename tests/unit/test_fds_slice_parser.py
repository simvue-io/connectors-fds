...
@pytest.mark.parametrize(
    "results_path",
    ("slice_singlemesh", "slice_multimesh"),
    ids=("single_mesh", "multi_mesh"),
)
@pytest.mark.parametrize(
    "slice_parameter",
    (["SOOT VISIBILITY", "TEMPERATURE"], None),
    ids=("visibility-temperature", "no-quantities"),
)
@pytest.mark.parametrize(
    "slice_fixed_dims", (["x", "y"], None), ids=("x-y", "no-quantities")
)
@pytest.mark.parametrize(
    "slice_ids",
    (["temperature_slice", "visibility_slice", "velocity_slice"], None),
    ids=("ids", "no-ids"),
)
        ...
        if load:
            ...
            run.launch(
                fds_input_file_path=pathlib.Path(__file__).parent.joinpath(
                    "example_data", results_path, "no_vents.fds"
                ),
                workdir_path=pathlib.Path(__file__).parent.joinpath(
                    "example_data", results_path
                ),
                slice_parse_enabled=enabled,
                slice_parse_quantities=slice_parameter,
                slice_parse_ids=slice_ids,
...
@pytest.mark.parametrize(
    "slice_id",
    (None, "cell_centered_slice", "large_slice", "wrong_id"),
    ids=("all_slices", "cell_centered_slice", "slice_too_big", "wrong_id"),
)
def test_fds_invalid_slice(folder_setup, caplog, slice_id):
    with FDSRun() as run:
        run.config(disable_resources_metrics=True)
        run.init(
            f"test_invalid_slice-{slice_id if slice_id else 'all_slices'}",
            folder=folder_setup,
        )
        run_id = run.id
        run.launch(
            fds_input_file_path=pathlib.Path(__file__).parent.joinpath(
                "example_data", "slice_invalid", "Pohlhausen_Pr_1.fds"
            ),
            workdir_path=pathlib.Path(__file__).parent.joinpath(
                "example_data", "slice_invalid"
            ),
    ...
    if slice_id == "cell_centered_slice" or not slice_id:
        assert all(
            ...
            in s
            for s in (caplog.text, events)
        )
    if slice_id == "large_slice" or not slice_id:
        assert all(
            "Slice 'large_slice' exceeds the maximum size for upload to the server - ignoring this metric."
            in s
            for s in (caplog.text, events)
        )
