...
@pytest.mark.parametrize(
    "slice_parse_enabled",
    (True, False),
    ids=("slice_parse_enabled", "slice_parse_disabled"),
)
@pytest.mark.parametrize(
    "file_name",
    (
        "fds_invalid_config.stderr",
        "fds_no_file.stderr",
        "fds_too_few_meshes.stderr",
        "fds_expected.stderr",
    ),
    ids=("invalid_config", "no_file", "cannot_mpi", "expected"),
)
...
def test_fds_stderr_check(folder_setup, file_name, slice_parse_enabled):
    ...
    def mock_execute_process(*args, file_name=file_name, **kwargs):
        ...
        def mock_process(
            result=_result, completion_callback=args[3], file_name=file_name
        ):
            while result.poll() is None:
                time.sleep(1)
            std_err = (
                pathlib.Path(__file__)
                .parent.joinpath("example_data", file_name)
                .read_text()
            )
    ...
    if slice_parse_enabled:
        assert any(
            f"Unable to load slice data found in output directory '{tempd}' - no simulation data found."
            in event
            for event in events
        )
