...
def test_chid_from_input_file():
    with tempfile.TemporaryDirectory() as temp_dir:
        shutil.copy(
            ...
            pathlib.Path(temp_dir).joinpath("test_chid.fds"),
        )
        pathlib.Path(temp_dir).joinpath("fds_test_1.sf.bnd").touch()

        with FDSRun() as run:
            run.init()
            run.load(temp_dir, upload_input_metadata=True)
...
def test_chid_invalid_results_files():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with FDSRun() as run:
            ...
            with pytest.raises(
                ValueError,
                match="Could not determine CHID from results directory due to files with inconsistent names.",
            ):
                run.load(temp_dir)
