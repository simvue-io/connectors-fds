...
def test_fds_supply_exhaust(folder_setup, offline_cache_setup, load, offline, parallel):
    if load:
        ...
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "load", "supply_exhaust_vents"
        )
    else:
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "launch", "supply_exhaust_vents.fds"
        )
    try:
        run_id = run_fds(
            file_path=file_path,
    ...
    assert any(
        [
            event.startswith("DEVC 'timer' has been set to 'True' at time 2.")
            for event in events
        ]
    )
...
def test_fds_aalto_woods(folder_setup, offline_cache_setup, offline, parallel, load):
    if load:
        if parallel:
            pytest.skip("Parallel has no effect when loading from historic runs")
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "load", "aalto_woods_spruce"
        )
    else:
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "launch", "aalto_woods_spruce", "spruce_N2_50.fds"
        )
    ...
    # Check events from log
    assert any(
        [event.startswith("Time Step: 1, Simulation Time: 0.5") for event in events]
    )
...
def test_fds_bre_spray(folder_setup, offline_cache_setup, offline, parallel, load):
    if load:
        if parallel:
            pytest.skip("Parallel has no effect when loading from historic runs")
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "load", "BRE_spray"
        )
    else:
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "launch", "BRE_Spray_A_1.fds"
        )
    ...
    assert any(
        [
            event.startswith("DEVC '1' has been set to 'True' at time 4.0")
            for event in events
        ]
    )
...
def test_fds_pohlhausen(folder_setup, offline_cache_setup, offline, parallel, load):
    if load:
        if parallel:
            pytest.skip("Parallel has no effect when loading from historic runs")
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "load", "pohlhausen"
        )
    else:
        file_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "launch", "Pohlhausen_Pr_1.fds"
        )
